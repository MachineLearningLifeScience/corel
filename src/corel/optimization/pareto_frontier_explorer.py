...
def make_pareto_frontier_explorer(dataset=None):
    ...
    def pareto_frontier_explorer(search_space: SearchSpaceType, acquisition_function) -> tf.Tensor:
        ...
        selected_seq, best_val = _get_best_of_single_site_mutations(proposal_seqs[:1, ...], acquisition_function)
        for i in range(1, len(proposal_seqs)):
            s, v = _get_best_of_single_site_mutations(proposal_seqs[i:i+1, ...], acquisition_function)
            if v > best_val:
                selected_seq = s
                best_val = v
        return selected_seq
